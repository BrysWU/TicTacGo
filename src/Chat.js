...
import {
  Box, TextField, IconButton, Typography, Stack, Fade
} from "@mui/material";
import SendIcon from "@mui/icons-material/Send";

const Chat = ({ socket, gameId, you, nicknames, disabled }) => {
  ...
  const getSenderName = (id) => {
    return id === you
      ? "You"
      : Object.entries(nicknames).find(([sym, sid]) => sid === id)
        ? nicknames.X === id ? nicknames.X : nicknames.O
        : "Opponent";
  };
        ...
        <Box sx={{ minHeight: 70, maxHeight: 110, overflowY: "auto", mb: 1, pr: 1 }}>
          ...
          {messages.map((msg, i) => (
            <Stack
              ...
              direction="row"
              spacing={1}
              sx={{ mb: 0.5, alignItems: "flex-end", justifyContent: msg.sender === you ? "flex-end" : "flex-start" }}
            >
              <Box
                sx={{
                  bgcolor: msg.sender === you ? "#ffb300" : "#00e5ff",
                  color: "#002147",
                  px: 1.5,
                  py: 0.7,
                  borderRadius: 2,
