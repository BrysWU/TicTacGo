...
const Board = ({ gameState, setGameState, onPlayAgain, nickname }) => {
  ...
  useEffect(() => {
    const handleUpdate = ({ board, turn, winner, winnerNickname }) => {
      setBoard([...board]);
      setTurn(turn);
      setWinner(winner);
    ...
    const handleLeft = () => setOpponentLeft(true);

    socket.on("gameUpdate", handleUpdate);
    socket.on("opponentLeft", handleLeft);

    return () => {
      socket.off("gameUpdate", handleUpdate);
      socket.off("opponentLeft", handleLeft);
    };
  }, [socket]);
          ...
          {opponentLeft ? (
            <Fade in>
              <Box sx={{ textAlign: "center", mb: 2 }}>
                ...
                <Button variant="contained" color="secondary" sx={{ mt: 3, fontWeight: 700 }} onClick={handlePlayAgain}>
                  Play Again
                </Button>
              </Box>
            </Fade>
