...
const Queue = ({ onGameStart, asGuest, guestNickname }) => {
  ...
  const joinQueue = () => {
    setErr("");
    if (asGuest) {
      socket.emit("joinQueue", { nickname: guestNickname });
    } else {
  ...
  return (
    <Fade in timeout={900}>
      <Box>
        ...
        {asGuest && (
          <Typography sx={{ mb: 1, fontWeight: 600, fontSize: 17, color: "#ffb300" }}>
            Playing as Guest: {guestNickname}
          </Typography>
        )}
